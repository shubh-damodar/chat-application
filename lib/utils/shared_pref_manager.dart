...
class SharedPrefManager {
  ...
  static Future<bool> setSavedContacts(String mapResponse) {
    //print('~~~ setSavedContacts: $mapResponse');
    return _sharedPreferences.setString('saved_contacts', mapResponse);
  }

  static Future<String> getSavedContacts() async {
    return _sharedPreferences.getString('saved_contacts');
  }
    ...
    if (allUsersString != null) {
      ...
      usersList.where((User iteratingUser) {
        if (iteratingUser.userId == updateUserId) {
          //print(
              // '~~~ where: ${iteratingUser.userId} $informationType $information');
          if (informationType == 'profileImage') {
            iteratingUser.profileImage = information;
